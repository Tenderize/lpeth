...
library UnsETHQueue {
     ...
     */
    function head(UnsETHQueue.Data storage q) internal view returns (Node memory) {
     ...
     */
    function tail(UnsETHQueue.Data storage q) internal view returns (Node memory) {
     ...
     */
    function popHead(UnsETHQueue.Data storage q) internal returns (Node memory node) {
     ...
     * @param q The queue to pop from
     */
    function popTail(UnsETHQueue.Data storage q) internal returns (Node memory node) {
     ...
     */
    function push(UnsETHQueue.Data storage q, Item memory unlock) internal {
        uint256 tail = q._tail;
        uint256 newTail = unlock.tokenId;

        if (tail != 0) {
            if (q.nodes[newTail].data.tokenId != 0) revert IdExists();
        }
